...
import type { EvidenceSentiment, EvidenceType } from "@/generated/prisma/enums";
...
export function signalWeight(item: EvidenceSignal): number {
  ...
  const directness = TYPE_DIRECTNESS[item.type] ?? 0.4;
  return strength * relevance * directness;
}

function isDirect(item: EvidenceSignal): boolean {
  return item.isDirectCustomer || DIRECT_TYPES.has(item.type);
}
...
export function computeEvidenceScore(
  ...
  const distinctTypes = new Set(supporting.map((i) => i.type)).size;
  const diversity = { fill: clamp(distinctTypes / 4, 0, 1), count: distinctTypes };
