...
/** Turn thrown errors into a safe, user-facing ActionResult in the user's locale. */
export async function safeAction<T>(name: string, fn: () => Promise<T>): Promise<ActionResult<T>> {
  try {
    ...
    const t = await getT();
    if (error instanceof UnauthorizedError) return { ok: false, error: t("shared.action.signIn") };
    if (error instanceof ForbiddenError) return { ok: false, error: t("shared.action.forbidden") };
    ...
    return { ok: false, error: t("shared.action.failed") };
  }
}
