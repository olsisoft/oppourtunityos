...
describe("computeNextActions", () => {
  ...
  it("RESEARCH with no evidence recommends research/economic impact, never building", () => {
    ...
    expect(a[0].title).toMatch(/economic impact of no-shows/);
    expect(
      a.some((x) => /before writing software|before thinking about solutions/.test(x.title)),
    ).toBe(true);
    expect(a.every((x) => !/build the product/i.test(x.title))).toBe(true);
  });
  ...
  it("prioritizes critical kill warnings and the riskiest assumption", () => {
    ...
    expect(a[1].title).toMatch(/owner controls software purchases/);
  });
  ...
  it("INTERVIEW recommends interviewing five ICPs and exploring mechanisms when few exist", () => {
    ...
    expect(a.some((x) => /Interview 5 salon owners/.test(x.title))).toBe(true);
  });

  it("TEST recommends a concierge test before software", () => {
    const a = computeNextActions({ ...base, verdict: "TEST" });
    expect(a.some((x) => x.type === "TEST" && /concierge/.test(x.title))).toBe(true);
  });
